...
frame_buffers = queue.Queue(maxsize=10)
...
async def mainLoop(websocket):
    ...
    try:
        ...
        while True:
            try:
                frame = frame_buffers.get(timeout=1)
                if frame is None:
...
async def ws_handler(websocket):
    """
    Handle WebSocket connections and start the frame producer.
    """
    stop_event = threading.Event()
    producer_thread = threading.Thread(
        target=frame_producer, args=(RTSP_URL, frame_buffers, stop_event), daemon=True
    )
